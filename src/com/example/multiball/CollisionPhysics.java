 ...
 */
/*
 * TODO: 3D support TODO: Error Analysis and Test cases TODO: assert in public
 * methods
 */
public class CollisionPhysics
{
	...
	/**
	 * Detect collision for a moving point bouncing inside a rectangular
	 * container, within the given timeLimit. If collision is detected within
	 * the timeLimit, compute collision time and response in the given
	 * CollisionResponse object. Otherwise, set collision time to infinity. The
	 * result is passed back in the given CollisionResponse object.
	 * 
	 * @param pointX
			...
			CollisionResponse response)
	{
		...
		// The CollisionResponse object keeps the result of one collision, not
		// both!
	}

	/**
	 * Detect collision for a moving point hitting a vertical line, within the
	 * given timeLimit. If collision is detected within the timeLimit, compute
	 * collision time and response in the given CollisionResponse object.
	 * Otherwise, set collision time to infinity. The result is passed back in
	 * the given CollisionResponse object.
	 * 
	 * @param pointX
	 *            : x-position of the center of the point.
	 * @param pointY
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLineVertical(float pointX, float pointY, float speedX, float speedY,
			float radius, float lineX, float timeLimit, CollisionResponse response)
	{
		...
		if (speedX == 0)
		{ // FIXME: Should I use a threshold?
			return;
		}
	 ...
	 * within the given timeLimit. If collision is detected within the
	 * timeLimit, compute collision time and response in the given
	 * CollisionResponse object. Otherwise, set collision time to infinity. The
	 * result is passed back in the given CollisionResponse object.
	 * 
	 * @param pointX
	 ...
	 *            : radius of the point. Zero for a true point.
	 * @param outerCenterX
	 *            : center y of the circular container.
	 * @param outerCenterY
	 *            : center x of the circular container.
	 * @param outerRadius
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsCircleOuter(float pointX, float pointY, float speedX, float speedY, float radius,
			float outerCenterX, float outerCenterY, float outerRadius, float timeLimit, CollisionResponse response)
	{
		...
		float t = pointIntersectsCircleOuterDetection(pointX, pointY, speedX, speedY, radius, outerCenterX,
				outerCenterY, outerRadius);
		// Accept 0 < t <= timeLimit
		if (t > 0 && t <= timeLimit)
		{
			...
			pointIntersectsLineNormalResponse(pointX, pointY, speedX, speedY, outerCenterX, outerCenterY, impactX,
					impactY, response, t);
		}
	}
	 ...
	 */
	private static float pointIntersectsCircleOuterDetection(float pointX, float pointY, float speedX, float speedY,
			float radius, float outerCenterX, float outerCenterY, float outerRadius)
	{
	 ...
	 * for a moving point and a line. The line is specified by its normal. Store
	 * and return the result in the given CollisionResponse object.
	 * 
	 * @param pointX
	 *            : x-position of the center of the point.
	 * @param pointY
	 ...
	 *            : speed in y-direction.
	 * @param lineNormalX1
	 *            : x1 of the normal to the collision line.
	 * @param lineNormalY1
	 *            : y1 of the normal to the collision line.
	 * @param lineNormalX2
	 *            : x2 of the normal to the collision line.
	 * @param lineNormalY2
	 *            : y2 of the normal to the collision line.
	 * @param response
	 *            : updated the collision time and response.
	 * @param t
	 *            : the given detected collision time.
	 */
	private static void pointIntersectsLineNormalResponse(float pointX, float pointY, float speedX, float speedY,
			float lineNormalX1, float lineNormalY1, float lineNormalX2, float lineNormalY2, CollisionResponse response,
			float t)
	{
		...
		// Reflect along the normal (N), no change along the line of collision
		// (P).
	...
	/**
	 * Detect collision for a moving point hitting a arbitrary polygon, within
	 * the given timeLimit. If collision is detected within the timeLimit,
	 * compute collision time and response in the given CollisionResponse
	 * object. Otherwise, set collision time to infinity. The result is passed
	 * back in the given CollisionResponse object.
	 * 
	 * @param pointX
	 *            : x-position of the center of the point.
	 * @param pointY
	 ...
	 *            : radius of the point. Zero for a true point.
	 * @param polygonXs
	 *            : array of x points of the polygon.
	 * @param polygonYx
	 *            : array of y points of the polygon.
	 * @param numPoints
	 *            : number of points to be taken from the point array.
	 * @param timeLimit
	 *            : max time to detect collision, in (0, 1] range.
	 * @param response
	 *            : If collision is detected, update the collision time and
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsPolygon(float pointX, float pointY, float speedX, float speedY, float radius,
			int[] polygonXs, int[] polygonYs, int numPoints, float timeLimit, CollisionResponse response)
	{
		...
		for (int segment = 0; segment < numPoints; segment++)
		{
			...
			CollisionPhysics.pointIntersectsLineSegmentNoEndPoints(pointX, pointY, speedX, speedY, radius, lineX1,
					lineY1, lineX2, lineY2, timeLimit, tempResponse);
			if (tempResponse.t < response.t)
			{
		...
		{
			CollisionPhysics.pointIntersectsPoint(pointX, pointY, speedX, speedY, radius, polygonXs[i], polygonYs[i],
					0, timeLimit, tempResponse);
			if (tempResponse.t < response.t)
			{
				response.copy(tempResponse);
			}
	 ...
	 *            : radius of the point. Zero for a true point.
	 * @param lineX1
	 *            : line's beginning point x value.
	 * @param lineY1
	 *            : line's beginning point y value.
	 * @param lineX2
	 *            : line's ending point x value.
	 * @param lineY2
	 *            : line's ending point y value.
	 * @param timeLimit
	 *            : max time to detect collision, in (0, 1] range.
	 * @param response
	 *            : If collision is detected, update the collision time and
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLine(float pointX, float pointY, float speedX, float speedY, float radius,
			float lineX1, float lineY1, float lineX2, float lineY2, float timeLimit, CollisionResponse response)
	{
		...
		if (lineX1 == lineX2)
		{ // Vertical line
			pointIntersectsLineVertical(pointX, pointY, speedX, speedY, radius, lineX1, timeLimit, response);
			return;
		} else if (lineY1 == lineY2)
		{ // Horizontal line
			pointIntersectsLineHorizontal(pointX, pointY, speedX, speedY, radius, lineY1, timeLimit, response);
			return;
		}
		...
		float t = pointIntersectsLineDetection(pointX, pointY, speedX, speedY, radius, lineX1, lineY1, lineX2, lineY2)[0];
		// Accept 0 < t <= timeLimit
		if (t > 0 && t <= timeLimit)
		{
			// Call helper method to compute the response in Response object
			pointIntersectsLineResponse(pointX, pointY, speedX, speedY, lineX1, lineY1, lineX2, lineY2, response, t);
		}
	}
	...
	/**
	 * Helper method to compute the collision time (t) and point of impact on
	 * the line (lambda), for a moving point and a line.
	 * 
	 * @param pointX
	 *            : x-position of the center of the point.
	 * @param pointY
	 ...
	 * @param lineY2
	 *            : line's ending point y value.
	 * @return an float[2], where First element is t, or infinity if no
	 *         collision detected. Second element is lambda, point of impact on
	 *         the line.
	 */
	private static float[] pointIntersectsLineDetection(float pointX, float pointY, float speedX, float speedY,
			float radius, float lineX1, float lineY1, float lineX2, float lineY2)
	{
		...
		if (radius > 0)
		{
			// Check which side of the line the point is. Offset reduces the
			// distance
		...
		// Solve for t (time of collision) and lambda (point of impact on the
		// line)
		double t;
		double lambda;
	...
	/**
	 * Helper method to compute the collision response given collision time (t)
	 * for a moving point and a line. Store and return the result in the given
	 ...
	 *            : line's ending point y value.
	 * @param response
	 *            : update collision time and response.
	 * @param t
	 *            : the given detected collision time
	 */
	private static void pointIntersectsLineResponse(float pointX, float pointY, float speedX, float speedY,
			float lineX1, float lineY1, float lineX2, float lineY2, CollisionResponse response, float t)
	{
	...
	/**
	 * Detect collision for a moving point hitting hitting an arbitrary line
	 * segment, within the given timeLimit. No need to consider the two end
	 * points. If collision is detected within the timeLimit, compute collision
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLineSegmentNoEndPoints(float pointX, float pointY, float speedX, float speedY,
			float radius, float lineX1, float lineY1, float lineX2, float lineY2, float timeLimit,
			CollisionResponse response)
	{
		...
		{ // Vertical line
			pointIntersectsLineVertical(pointX, pointY, speedX, speedY, radius, lineX1, timeLimit, response);
			// Need to confirm that the point of impact is within the
			// line-segment
		...
		// Call helper method to compute the collision time.
		float[] result = pointIntersectsLineDetection(pointX, pointY, speedX, speedY, radius, lineX1, lineY1, lineX2,
				lineY2);
		float t = result[0];
	...
	/**
	 * Detect collision for a moving point hitting hitting an arbitrary line
	 * segment, within the given timeLimit. Consider both the end points. If
	 * collision is detected within the timeLimit, compute collision time and
	 * response in the given CollisionResponse object. Otherwise, set collision
	 * time to infinity. The result is passed back in the given
	 * CollisionResponse object.
	 * 
	 * @param pointX
	 *            : x-position of the center of the point.
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLineSegment(float pointX, float pointY, float speedX, float speedY, float radius,
			float lineX1, float lineY1, float lineX2, float lineY2, float timeLimit, CollisionResponse response)
	{
		...
		CollisionPhysics.pointIntersectsLineSegmentNoEndPoints(pointX, pointY, speedX, speedY, radius, lineX1, lineY1,
				lineX2, lineY2, timeLimit, tempResponse);
		if (tempResponse.t < response.t)
		{
			response.copy(tempResponse);
		}
		// Check the two end points (with radius = 0) for probable collision
		CollisionPhysics.pointIntersectsPoint(pointX, pointY, speedX, speedY, radius, lineX1, lineY1, 0, timeLimit,
				tempResponse);
		if (tempResponse.t < response.t)
		{
			response.copy(tempResponse);
		}
		CollisionPhysics.pointIntersectsPoint(pointX, pointY, speedX, speedY, radius, lineX2, lineY2, 0, timeLimit,
				tempResponse);
		if (tempResponse.t < response.t)
		{
			response.copy(tempResponse);
	...
	/**
	 * Detect collision for a moving point hitting another moving point, within
	 * the given timeLimit. If collision is detected within the timeLimit,
	 * compute collision time and response in the given CollisionResponse
	 * object. Otherwise, set collision time to infinity. The result is passed
	 * back in the given CollisionResponse object.
	 ...
	 * @param p1Radius
	 *            : p1's radius. Zero for a true point.
	 * @param p2X
	 ...
	 * @param p1Response
	 *            : If collision is detected, update the collision time and
	 *            response for p1. Otherwise, set collision time to infinity.
	 * @param p2Response
	 *            : If collision is detected, update the collision time and
	 *            response for p2. Otherwise, set collision time to infinity.
	 */
			...
			CollisionResponse p1Response, CollisionResponse p2Response)
	{
		...
		if (t > 0 && t <= timeLimit)
		{
			// Call helper method to compute the responses in the 2 Response
			// objects
	...
	/**
	 * Helper method to compute the collision response given the collision time
	 * (t), for two moving points. Store and return the results in the two given
	 ...
	 *            : p2's radius. Zero for a true point.
	 * @param p1Response
	 *            : To update the collision time and response for p1. Reset time
	 *            to infinity if error is detected.
	 * @param p2Response
	 *            : To update the collision time and response for p2. Reset time
	 *            to infinity if error is detected.
	 * @param t
	 *            : the given detected collision time.
	 */
			...
			CollisionResponse p1Response, CollisionResponse p2Response, float t)
	{
		...
		// Do not declare collision, so that they continue their course of
		// movement
		...
		// Along the collision direction P, apply conservation of energy and
		// momentum
	...
	/**
	 * Detect collision for a moving point hitting a stationary point, within
	 * the given timeLimit. If collision is detected within the timeLimit,
	 * compute collision time and response in the given CollisionResponse
	 * object. Otherwise, set collision time to infinity. The result is passed
	 * back in the given CollisionResponse object.
	 ...
	 *            response for p1. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsPoint(float p1X, float p1Y, float p1SpeedX, float p1SpeedY, float p1Radius,
			float p2X, float p2Y, float p2Radius, float timeLimit, CollisionResponse p1Response)
	{
		...
		float t = pointIntersectsMovingPointDetection(p1X, p1Y, p1SpeedX, p1SpeedY, p1Radius, p2X, p2Y, 0f, 0f,
				p2Radius);

		// Accept 0 < t <= timeLimit
		if (t > 0 && t <= timeLimit)
		{
			// Call helper method to compute and return the response given
			// collision time t.
	...
	/**
	 * Helper method to compute the collision response given the collision time
	 * (t), for a moving point hitting a stationary point. Store and return the
	 * results in the given CollisionResponse object.
	 * 
	 * @param p1X
	 *            : x-position of the center of point p1.
	 * @param p1Y
	 ...
	 *            : the given detected collision time.
	 */
	private static void pointIntersectsPointResponse(float p1X, float p1Y, float p1SpeedX, float p1SpeedY, float p2X,
			float p2Y, CollisionResponse p1Response, float t)
	{
	...
	/**
	 * Helper method to rotation vector (x, y) by theta, in Graphics
	 * coordinates. y-axis is inverted. theta measured in counter-clockwise
	 * direction. Re-use the double[] rotateResult to avoid repeated new
	 * operations.
	 * 
	 * @param x
	 ...
	 * @return An double array of 2 elements x and y, in the rotated
	 *         coordinates.
	 */
